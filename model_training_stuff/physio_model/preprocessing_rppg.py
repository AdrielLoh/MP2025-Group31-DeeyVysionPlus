...
def preprocess_video_worker(args):
    video_path, window_size, hop_size, fs = args
    ...
    for track_id, track in tracks.items():
        ...
        for win in rgb_windows:
            rppg_feat = compute_rppg_features(win, fs)
...
def cache_batches_parallel(video_dir, label, class_idx, batch_size=128, cache_dir='cache/batches/train',
                          window_size=150, hop_size=75, fs=30, max_workers=None):
    ...
    batch_idx = 0
    tasks = [(video_path, window_size, hop_size, fs) for video_path in file_list]
    if max_workers:
...
if __name__ == "__main__":
    cache_batches_parallel(
        ...
        fs=30  # Use your dataset's actual frame rate
    )
    cache_batches_parallel(
