...
N_MELS = 128
N_MFCC = 20
TRAIN_PATH = "G:/deepfake_training_datasets/training_audio/training"
TEST_PATH = "G:/deepfake_training_datasets/training_audio/testing"
REPLACEMENT_REAL_PATH = "G:/deepfake_training_datasets/Common-Voice-Bonafide-Only/clips"
REPLACEMENT_FAKE_PATH = "G:/deepfake_training_datasets/ASVspoof-2021/ASVspoof2021_DF_eval/separated/spoofed"
...
def process_single_file(args):
    ...
    try:
        ...
        if label == 0 and random.random() < 0.2: # 0.4
            y_audio = augment_audio(y_audio, sr)
        elif label == 1 and random.random() < 0.5: 
            y_audio = augment_audio(y_audio, sr)
        features, avg_energy = extract_combined_features(y_audio)
        if avg_energy < SILENCE_THRESHOLD:
            raise Exception("Sample too silent or low-energy")
...
def preprocess_and_cache_balanced(real_samples, fake_samples, scaler, output_dir):
    batch_index = 0
...
if __name__ == "__main__":
    ...
    preprocess_and_cache_balanced(train_real_pairs, train_fake_pairs, scaler, TRAIN_CACHE_DIR)
    print("[*] Starting validation batch preprocessing...")
    preprocess_and_cache_balanced(val_real, val_fake, scaler, VAL_CACHE_DIR)
